	...
	$('#messages-stack-list').off('click').on("click", ".load-more-contacts", function() {
		...
		if (ID) {
			$.ajax({
				type: "POST",
				url: URL,
				data: {lastid: ID, uid: UID, token: token, tab: $.tab},
				cache: false,
				beforeSend: function() {
	...
	// Messages System
	$('#messages-stack-list').on("click", "li.prepare-message", function(e) {
		e.preventDefault();
		var ID = $(this).attr("id");
	...
	$("#text-messages-request").on("focus", "textarea.type-a-message-box", function() { 
		...
		$('div.message-btn-target').html('<span id="'+ID+'" class="btn btn-default btn-sm send-message"><i class="glyphicon glyphicon-send"></i> '+$.sendButton+'</span>');
		...
		if($.enterIsSend == true)
		{
			...
			$("span.send-message").on("click", function(e) {
				e.preventDefault();
				send_message(ID, URL);	
			});
	...
	{ 
		...
		if ($.trim(textarea).length == 0) 
		{
			...
			$.ajax({
				type: "POST",
				url: URL,
				data: {id: ID, token: tkn, message: encodeURIComponent(textarea)},
				cache: false,
				beforeSend: function() { $('#loadingDiv').show(); },
				error: function() { $('#loadingDiv').hide(); $('#errorDiv').html('<p>'+$.ajaxError+'</p>'); },
				success: function(html) {
	...
	$('body').off('click').on("click", ".load-more-messages", function(e) {
		e.preventDefault();
		var ID = $(this).attr("id");
		var UID = $(this).attr("rel");
		...
		if (ID) {
			$.ajax({
				type: "POST",
				url: URL,
				data: {lastid: ID, token: token, uid: UID},
				cache: false,
				beforeSend: function() {
	...
	$('#contacts-search').on("click", "#contacts-search-input", function(e) {
		e.preventDefault();
		...
		$(this).keyup(function() {
			...
			if ($.trim(filterbox).length > 0) {
				$.ajax({
					type: "POST",
					url: URL,
					data: {filterword: filterbox, token: token},
					cache: false,
					beforeSend: function() { $('#loadingDiv').show(); },
					error: function() { $('#loadingDiv').hide(); $('#errorDiv').html('<p>'+$.ajaxError+'</p>'); },
					success: function(html) {
	...
	$('#text-messages-request').off("click").on("click", ".remove-message", function(e) {
		e.preventDefault();
		var ID = $(this).attr("id");
		...
		$.ajax({
				type: "POST",
				url: URL,
				data: {id: ID, token: token, uid: UID},
				cache: false,
				beforeSend: function() { $('#loadingDiv').show(); },
				error: function() { $('#loadingDiv').hide(); $('#errorDiv').html('<p>'+$.ajaxError+'</p>'); },
				success: function(html) {
	...
	function title_unread_counter() {
		...
		$.ajax({
				type: "POST",
				url: URL,
				data: {total_unread: 'true', token: token},
				cache: false,
	...
	// Emoticons
	$('body').on('click', '#emoticons', function(e) {
		e.preventDefault();
	...
	function uploadify_init(extension, attachext, upType)
	{
		...
		$(function() {
			$('#file_upload').uploadify({
				...
				{ 
					var value = current_value+'['+attachext+'-'+tm+file.name+']';
					$("textarea.type-a-message-box").val(value.toString());
	...
	// Send Photo
	$('body').on('click', '#send-photo', function(e) {
		e.preventDefault();
	...
	// Send File
	$('body').on('click', '#send-file', function(e) {
		e.preventDefault();
	...
	// Send Location
	$('body').on('click', '#send-location', function(e) {
		e.preventDefault();
		...
		$('.modal-header h4.modal-title').html('Send Location');
		$('.modal-body').html(
			'<article></article>'+
			'<input type="hidden" value="" class="form-control input-sm" id="maps_latitude">'+
			'<input type="hidden" value="" class="form-control input-sm" id="maps_longitude">'
		);
...
function last_seen()
{
	$(window).bind("beforeunload", function() {
		var token = $('#token').val();
		$.post($.base_url + 'ajax/ajax_last_seen.php', 'offline=true&token='+token, function(response){}); 	
	}); 
	
	$(window).focus(function() {
		var token = $('#token').val();
		$.post($.base_url + 'ajax/ajax_last_seen.php', 'offline=false&token='+token, function(response){}); 
	});
}
...
{
	var token = $('#token').val();
	$.post(R_URL, dataString+'&token='+token, function(response) 
	{
...
function contacts_tab()
{
	...
	$.ajax({
		type: "POST",
		url: URL,
		data: {post_tabs: 'contacts', token: token},
		cache: false,
...
{	
	...
	$.post(URL, dataString+'&token='+token, function(html) {
...
function success(position) 
{
  ...
  $('#send-location-btn').show();
  
  $('#maps_latitude').val(position.coords.latitude);
  $('#maps_longitude').val(position.coords.longitude);
  ...
  $('#send-location-btn').on('click', function(e) {
	e.preventDefault();
...
{ 
   ...
   geocoder.geocode({
		...
		if (status == google.maps.GeocoderStatus.OK) 
		{
			...
			$('#maps_latitude').val(lat);
			var lng = results[0].geometry.location.lng();
			$('#maps_longitude').val(lng);
		} 
	});
}
